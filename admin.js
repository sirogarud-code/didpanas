...
function fillReviewDetails(review) {
  ...
  $("#revFormName").value   = review.name   || "";
  $("#revFormCity").value   = review.city   || "";
  $("#revFormPet").value    = review.pet    || "";
  $("#revFormRating").value = review.rating != null ? review.rating : "";
  $("#revFormText").value   = review.text   || "";

  // дата → datetime-local
  const dateInput = $("#revFormDate");
  if (dateInput) {
    if (review.createdAt) {
...
function clearReviewFormNew() {
  ...
  $("#revFormRating").value = "";
  $("#revHint").textContent =
    "Створення нового відгуку. Заповніть форму й натисніть «Зберегти».";
}
...
function initReviewsSection() {
  ...
  // клік по таблиці – вибір існуючого відгуку
  ...
  // збереження (новий або редагування)
  $("#reviewFormAdmin")?.addEventListener("submit", e => {
    e.preventDefault();

    const name   = ($("#revFormName").value || "").trim() || "Анонім";
    const city   = ($("#revFormCity").value || "").trim();
    const pet    = ($("#revFormPet").value || "").trim();
    const text   = ($("#revFormText").value || "").trim();
    let   rating = Number($("#revFormRating").value || 5);
    const dateVal = ($("#revFormDate").value || "").trim();
    const hint = $("#revHint");

    if (!text) {
      hint.textContent = "Введіть текст відгуку.";
      return;
    }
    ...
    if (!createdAt) {
      // якщо редагуємо – залишаємо стару дату
      if (currentReviewIndex != null && REVIEWS[currentReviewIndex]) {
        createdAt = REVIEWS[currentReviewIndex].createdAt || new Date().toISOString();
      } else {
        createdAt = new Date().toISOString();
      }
    }

    const obj = { name, city, pet, rating, text, createdAt };
    ...
    saveReviews();
    renderReviewsTable();
    fillReviewDetails(REVIEWS[currentReviewIndex]);
    hint.textContent = "Відгук збережено.";
  });
