...
// ---------- Застосування налаштувань до футера ----------

function applyFooterSettingsToPage() {
  const s = loadFooterSettings();

  const contactsList = document.querySelector(".sf-contacts");
  if (!contactsList) return;

  const phoneLink   = contactsList.querySelector("a[href^='tel:']");
  const emailLink   = contactsList.querySelector("a[href^='mailto:']");
  const deliveryLi  = contactsList.querySelector("li:nth-child(3)");
  ...
  // опис доставки
  if (deliveryLi && s.delivery) {
    deliveryLi.textContent = "🚚 " + s.delivery;
  }
}
...
/* ===== Sync reviews with admin (cross-tab) ===== */
window.addEventListener("storage", (e) => {
  // реагуємо тільки на зміну ключа з відгуками
  if (e.key !== LS_REVIEWS) return;

  try {
    userReviews = e.newValue ? JSON.parse(e.newValue) : [];
  } catch {
    userReviews = [];
  }

  // скидаємо на першу сторінку і перемальовуємо блок відгуків
  reviewsPage = 1;
  renderReviews();
});
...
function renderGrid() {
  ...
  updateBadges();
  applyFooterSettingsToPage();   // 🆕 підтягнути контакти з адмінки
  initNavScroll();
...
// ✅ ВИПРАВЛЕНА ФУНКЦІЯ
// Тепер показуємо ТІЛЬКИ ті відгуки, що в localStorage (zf_reviews)
...
// Більше не додаємо демо-відгуки автоматично
...
const FOOTER_SETTINGS_PUBLIC = loadFooterSettings();
...
document.querySelectorAll("[data-social]").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    const handle =
      FOOTER_SETTINGS_PUBLIC.socials &&
      FOOTER_SETTINGS_PUBLIC.socials[key]
        ? FOOTER_SETTINGS_PUBLIC.socials[key].trim()
        : "";

    let url = base;

    if (handle) {
      if (key === "instagram" || key === "facebook" || key === "telegram") {
        const clean = handle.replace(/^@/, "");
        url = base + clean;
      } else if (key === "viber") {
        const clean = handle.replace(/[^\d+]/g, "");
        if (clean) {
          url = `${base}?number=${encodeURIComponent(clean)}`;
        }
      }
    }
...
// показуємо кнопку при прокрутці вниз
